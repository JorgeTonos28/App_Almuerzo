...
3.  **Instalar Base de Datos**:
    ...
    *   Ejecuta la función `setupSheetsAndConfig()`. Esto creará automáticamente las pestañas necesarias (`Config`, `Usuarios`, `Menu`, `Pedidos`, `DiasLibres`) y cargará datos de ejemplo.
4.  **Habilitar APIs**:
    *   En el editor de Apps Script, ve a "Servicios" (+).
    *   Selecciona **Google Calendar API** y añádela.
...
5.  **Ajustar Configuración**:
    ...
    *   Configura `BACKUP_FOLDER_ID` con el ID de una carpeta de Drive donde se guardarán los respaldos diarios.
    *   Define `RESPONSIBLES_EMAILS_JSON` si deseas reportes por departamento (Ej: `{"Finanzas": "jefe@fin.com", "RRHH": "dir@rrhh.com"}`).
...
6.  **Configurar Automatización (Triggers)**:
    *   En el editor de Apps Script, ve a **Activadores** (Triggers).
    *   Añade los siguientes disparadores basados en tiempo (Temporizador):
        *   `scheduledSendReminders` -> Diario -> 1:00 PM a 2:00 PM.
        *   `scheduledLockSheet` -> Diario -> 2:00 PM a 3:00 PM (Opcional, bloqueo lógico).
        *   `scheduledDailyClose` -> Diario -> 2:00 PM a 3:00 PM (Ajustar hora exacta si es posible a 2:30 PM).
        *   `scheduledDepartmentReports` -> Diario -> 3:00 PM a 4:00 PM.
