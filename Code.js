 ...
 */
const APP_VERSION = 'v4.8-Admin-Polished';
...
function apiGetAdminData() {
  ...
  // Fix: Include 'creado_por' (col 11 -> index 10)
  const pSh = SpreadsheetApp.getActive().getSheetByName('Pedidos');
  const pData = pSh.getDataRange().getValues();
  const nowStr = formatDate_(new Date());
  data.orders = pData.slice(1)
    // Removed date filter to allow "All Orders" view if needed, but for performance keep future?
    // User asked for filters including "Range of date". So we should probably return ALL relevant orders (maybe last 30 days?)
    // or return logic to filter on server. The user said "server side like" but implies client update.
    // Let's allow fetching more. For now, we return active/future to keep payload small,
    // or maybe last 30 days. Let's stick to "Future + Today" for default, but maybe we need an endpoint for historical?
    // User said: "mostrar una sección de filtros... fecha única o rango...".
    // If I return only future, they can't see past orders.
    // I will return last 60 days for now to be safe without pagination logic yet.
    .filter(r => {
  ...
  if (user.rol === 'ADMIN_GEN') {
     ...
     // Config keys array for frontend form
